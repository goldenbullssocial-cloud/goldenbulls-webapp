  ...
  useEffect(() => {
    ...
    if (callback && planData) {
      try {
        ...
        console.log(plan, "decoded plan from URL");
        // Store plan data in sessionStorage for after login
        sessionStorage.setItem("selectedPlan", JSON.stringify(plan));
        sessionStorage.setItem("callbackUrl", callback);
        // Store scroll parameter if present
        if (scroll) {
          sessionStorage.setItem("scrollParam", scroll);
        }
      } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Check if there's a callback URL and plan data
        ...
        if (callbackUrl && selectedPlan) {
          ...
          const redirectUrl = scrollParam
            ? `${callbackUrl}?scroll=${scrollParam}`
            : callbackUrl;
