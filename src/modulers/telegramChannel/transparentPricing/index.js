...
export default function TransparentPricing() {
    ...
    const processPayment = async () => {
        ...
        try {
            ...
            if (appliedCoupon) {
                const couponDiscountAmount = (originalPrice * appliedCoupon.discount) / 100;
            ...
            const paymentData = {
                ...
                walletAmount: useWalletBalance ? Math.min(user?.earningBalance || 0, finalPrice) : 0,
                actualAmount: useWalletBalance ? Math.max(0, finalPrice - (user?.earningBalance || 0)) : finalPrice,
                price: finalPrice,
            };
                        ...
                        <div className={styles.modalBody}>
                            ...
                            <div className={styles.planDetails}>
                                ...
                                <div className={`${styles.priceRow} ${styles.total}`}>
                                    ...
                                    <span className={styles.value}>${calculateTotalAmount()}</span>
                                </div>
                            </div>
                            ...
                            <div className={styles.subscribeSection}>
                                <Button
                                    text={loading ? "Processing..." : `Pay $${calculateTotalAmount()}`}
                    ...
                    <div className={styles.confirmModalContent} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={styles.confirmBody}>
                            <ul className={styles.confirmList}>
                                <li>
                                    Are you sure you want to use your wallet balance of <span className={styles.goldAmount}>${(user?.earningBalance || 0).toFixed(2)}</span>?
                                </li>
                                {user?.earningBalance >= parseFloat(calculateTotalAmount()) ? (
                                    <li>
                                        Your wallet balance covers the full purchase. <span className={styles.goldAmount}>${calculateTotalAmount()}</span> will be deducted from your wallet and the remaining <span className={styles.goldAmount}>${(user?.earningBalance - parseFloat(calculateTotalAmount())).toFixed(2)}</span> will remain in your wallet.
                                    </li>
                                ) : (
                                    <li>
                                        This amount <span className={styles.goldAmount}>${(user?.earningBalance || 0).toFixed(2)}</span> will be deducted from your wallet and the remaining <span className={styles.goldAmount}>${(parseFloat(calculateTotalAmount()) - (user?.earningBalance || 0)).toFixed(2)}</span> will be charged to your payment method.
                                    </li>
                                )}
                            </ul>
