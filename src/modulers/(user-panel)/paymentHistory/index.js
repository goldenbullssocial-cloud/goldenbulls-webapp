...
export default function PaymentHistory() {
  ...
  const handleSaveMetaAccount = async () => {
    ...
    try {
      ...
      setMetaAccountError(
        "Failed to save meta account number. Please try again.",
      );
    } finally {
      setSavingMeta(false);
    }
  ...
  //       ],
  //       couponDiscount:
  //         payment.couponDiscount > 0 ? `${payment.couponDiscount}` : "0",
  //       planDiscount:
  //         payment.discount > 0
  //           ? `${((payment.price * payment.discount) / 100).toFixed(2)}`
  //           : "0",
  //       totalValue: (
  //         parseFloat(payment.initialPrice * payment.noOfBots || payment.price) ||
  //         0
  //       ).toString(),
  ...
  const handleDownloadInvoice = async (payment) => {
    ...
    try {
      ...
      const invoicePayload = {
        ...
        totalValue: (
          parseFloat(
            payment.initialPrice * payment.noOfBots || payment.price,
          ) || 0
        ).toString(),
        total: (parseFloat(payment.price) || 0).toString(),
        invoiceNo: payment?.invoiceNo || "N/A",
                ...
                <tr key={item._id || index}>
                  ...
                  <td>
                    {item.courseId?.CourseName ||
                      ...
                      item?.telegramId?.telegramId?.channelName ||
                      "-"}
                  </td>
                  ...
                  <td>
                    {!item.courseId?.courseType && !item?.telegramId ? (
                      (Array.isArray(item.metaAccountNo) &&
                        item.metaAccountNo.length === 0) ||
                      !item.metaAccountNo ? (
                        <button
                          className={styles.cancelBtn}
                          onClick={() => handleOpenMetaModal(item, false)}
                        >
                          <span>Add</span>
                        </button>
                      ) : (
                        <button
                          className={styles.cancelBtn}
                          onClick={() => handleOpenMetaModal(item, true)}
                        >
                  ...
                  <td>
                    <span
                      className={
                        item.status === "paid" ? styles.green : styles.red
                      }
                    >
        ...
        <div className={styles.modalOverlay} onClick={handleCloseMetaModal}>
          ...
          >
            <div className={styles.modalHeader}>
              <h3>
                {viewMode ? "Meta Account Number" : "Add Meta Account Numbers"}
              </h3>
              <button
            ...
            <div className={styles.modalBody}>
              {viewMode ? (
                ...
                Array.isArray(selectedPayment?.metaAccountNo) &&
                  ...
                  selectedPayment.metaAccountNo.map((accountNo, index) => (
                    <div className={styles.inputGroup} key={index}>
                      ...
                      <div className={styles.inputWithButton}>
                        <input type="text" value={accountNo} disabled={true} />
                      </div>
                    </div>
                  ))
                ) : (
