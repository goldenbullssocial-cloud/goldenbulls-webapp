...
export default function CoursesDetailsBanner() {
  ...
  return (
    <motion.div className={styles.coursesDetailsBanner} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }}>
      <div className={styles.box}>
        <div className="container-md">
          <div className={styles.grid}>
            ...
            <motion.div className={styles.griditems} variants={container}>
              ...
              {courseType === "physical" && course?.isPayment === true && (
                <div className={styles.locationInfo}>
                  ...
                  <span>{course?.payment?.[0]?.batchId?.centerId ? `${course.payment[0].batchId.centerId.centerName}, ${course.payment[0].batchId.centerId.city}, ${course.payment[0].batchId.centerId.state}, ${course.payment[0].batchId.centerId.country}` : "Location info not available"}</span>
                </div>
              )}

              <motion.div className={styles.leftBottomAlignment} variants={fadeUp}>
                <div className={styles.time}>
                  <ClockIcon />
                  <span>{courseType === "live" || courseType === "physical" ? `${course?.chapter?.length || 0} Days` : `${course?.hours} Hours`}</span>
                </div>
            ...
            {/* RIGHT CARD */}
            <motion.div className={styles.griditems} variants={cardAnim} whileHover={{ y: -6 }} transition={{ duration: 0.3 }}>
              <div className={styles.card}>
                <div className={styles.cardImage}>
                  <div className={styles.image}>{course?.courseIntroVideo ? getYoutubeId(course.courseIntroVideo) ? <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${getYoutubeId(course.courseIntroVideo)}?autoplay=1&mute=1&loop=1&controls=1&playlist=${getYoutubeId(course.courseIntroVideo)}`} title="Course Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ borderRadius: "12px", position: "absolute", top: 0, left: 0, width: "100%", height: "100%" }}></iframe> : <video src={course.courseIntroVideo} poster={course?.courseVideo || CourseImage} autoPlay muted loop playsInline controls width="100%" style={{ borderRadius: "12px", objectFit: "cover", height: "100%" }} /> : <img src={course?.courseVideo || CourseImage} alt="CourseImage" />}</div>
                </div>
                ...
                <div className={styles.details}>
                  <div className={styles.contentAlignment}>
                    ...
                    <h3>{course?.isFree !== true && ("$" + course?.price)}</h3>
                    <ul>
